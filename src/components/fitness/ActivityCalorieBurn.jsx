    ...
    <div className="px-5">
      ...
      <div className="rounded-3xl bg-white border border-[#DBEAFE] p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {intensityLevels.map((lvl) => (
              <button
                ...
                className={`rounded-xl p-2.5 text-left transition-colors ${form.intensity === lvl.value ? 'bg-[#2563EB] text-white' : 'bg-[#EFF6FF] text-[#64748B]'}`}
              >
        ...
        <button
          onClick={calculate}
          disabled={loading}
          className="w-full rounded-full bg-[#BFDBFE] text-[#0F172A] py-3 font-semibold text-sm flex items-center justify-center gap-2 disabled:opacity-50 hover:bg-[#93C5FD] transition-colors"
        >
...
function CalorieStat({ minutes, calories, highlighted }) {
  return (
    <div className={`rounded-xl p-2.5 text-center ${highlighted ? 'bg-[#BFDBFE]/40' : 'bg-[#EFF6FF]'}`}>
