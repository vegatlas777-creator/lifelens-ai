...
export default function WeightTracker() {
  ...
  const diffColor = diff < 0 ? 'text-emerald-500' : diff > 0 ? 'text-rose-500' : 'text-[#666]';
  ...
  return (
    <div className="px-5 space-y-4">
      ...
      <div className="rounded-3xl bg-white border border-[#F5EFE6] p-5">
        <h3 className="text-sm font-bold text-[#1A1A1A] mb-3">Log New Weight</h3>
        <input type="number" step="0.1" value={weight} onChange={(e) => setWeight(e.target.value)} placeholder="Weight in kg" className="w-full rounded-xl bg-[#FDF6EE] border border-[#F5EFE6] px-3 py-2.5 text-sm focus:outline-none focus:border-[#FF9F43] text-[#1A1A1A] mb-2" />
        <input type="text" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Notes (optional)" className="w-full rounded-xl bg-[#FDF6EE] border border-[#F5EFE6] px-3 py-2.5 text-sm focus:outline-none focus:border-[#FF9F43] text-[#1A1A1A] mb-3" />
        <button onClick={addEntry} disabled={adding || !weight} className="w-full rounded-full bg-[#FFD5A8] text-[#1A1A1A] py-3 font-semibold text-sm flex items-center justify-center gap-2 disabled:opacity-50">
      ...
      {chartData.length > 1 && (
        <div className="rounded-3xl bg-white border border-[#F5EFE6] p-5">
          <h3 className="text-sm font-bold text-[#1A1A1A] mb-3">Weight Progress</h3>
          <div className="h-48">
      ...
      {entries.length > 0 && (
        <div className="rounded-3xl bg-white border border-[#F5EFE6] p-5">
          <h3 className="text-sm font-bold text-[#1A1A1A] mb-3">History</h3>
          <div className="space-y-2">
            {entries.map((e, i) => (
              <div key={e.id || i} className="flex items-center justify-between py-2 border-b border-[#F5EFE6] last:border-0">
                <div>
                  <p className="text-sm font-medium text-[#1A1A1A]">{e.weight_kg} kg</p>
                  <p className="text-xs text-[#666]">{formatDate(e.entry_date)}{e.notes ? ` · ${e.notes}` : ''}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {entries.length === 0 && (
        <div className="rounded-2xl border border-dashed border-[#FDDDBD] p-8 text-center">
          <p className="text-sm text-[#666]">No weight entries yet. Add your first entry above!</p>
        </div>
      )}
    </div>
  );
...
function StatCard({ label, value, icon: Icon, color }) {
  return (
    <div className="rounded-2xl bg-white border border-[#F5EFE6] p-3 text-center">
      ...
      <p className={`text-base font-bold ${color || 'text-[#1A1A1A]'} flex items-center justify-center gap-1`}>
