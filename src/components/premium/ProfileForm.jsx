    ...
    <div className="px-5 space-y-4">
      <div className="rounded-3xl bg-white border border-[#FFC0D6] shadow-sm shadow-pink-200/60 p-5 space-y-4">
        <div className="flex items-center gap-2 mb-1">
          <User size={18} className="text-[#FF149C]" />
          <h2 className="text-base font-bold text-[#4A0E2E] font-heading">Personal Information</h2>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="Gender">
            <div className="flex gap-2">
              {['male', 'female'].map((g) => (
                <button key={g} onClick={() => setForm({ ...form, gender: g })} className={`flex-1 py-2.5 rounded-xl text-sm font-medium capitalize ${form.gender === g ? 'bg-[#E91E63] text-white' : 'bg-[#FFD9E6] text-[#B0407A]'}`}>{g}</button>
              ))}
            </div>
          </Field>
        </div>
      ...
      <div className="rounded-3xl bg-white border border-[#FFC0D6] shadow-sm shadow-pink-200/60 p-5 space-y-4">
        <h2 className="text-base font-bold text-[#4A0E2E] font-heading">Daily Goals</h2>
      ...
      <button onClick={save} disabled={saving} className="w-full rounded-full bg-gradient-to-r from-[#FF69B4] to-[#FF149C] text-white py-3.5 font-semibold text-sm flex items-center justify-center gap-2 disabled:opacity-50 shadow-md shadow-pink-300/50">
...
function Field({ label, children }) {
  return (<div><label className="text-xs font-medium text-[#B0407A] mb-1.5 block">{label}</label>{children}</div>);
}