    ...
    <div className="px-5 space-y-4">
      <div className="rounded-3xl bg-white border border-[#F5EFE6] p-5">
        <div className="flex items-center gap-2 mb-3">
          <Ruler size={16} className="text-[#FF9F43]" />
          <h3 className="text-sm font-bold text-[#1A1A1A]">Log Body Measurements (cm)</h3>
        </div>
        ...
        <button onClick={addEntry} disabled={adding} className="w-full mt-3 rounded-full bg-[#FFD5A8] text-[#1A1A1A] py-3 font-semibold text-sm flex items-center justify-center gap-2 disabled:opacity-50">
      ...
      {entries.length > 0 ? (
        <div className="rounded-3xl bg-white border border-[#F5EFE6] p-5">
          <h3 className="text-sm font-bold text-[#1A1A1A] mb-3">Measurement History</h3>
          <div className="space-y-3">
            {entries.map((e, i) => (
              <div key={e.id || i} className="rounded-xl bg-[#FDF6EE] p-3">
                <p className="text-xs font-medium text-[#666] mb-2">{formatDate(e.entry_date)}</p>
                <div className="grid grid-cols-5 gap-1 text-center">
                  {['chest_cm', 'waist_cm', 'hips_cm', 'arm_cm', 'thigh_cm'].map((k) => (
                    <div key={k}>
                      <p className="text-[9px] text-[#999] uppercase">{k.replace('_cm', '')}</p>
                      <p className="text-sm font-medium text-[#1A1A1A]">{e[k] ? `${e[k]}` : '—'}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="rounded-2xl border border-dashed border-[#FDDDBD] p-8 text-center">
          <p className="text-sm text-[#666]">No measurements logged yet.</p>
        </div>
      )}
    </div>
  );
...
function Field({ label, children }) {
  return (<div><label className="text-[10px] font-medium text-[#666] mb-1 block">{label}</label>{children}</div>);
}