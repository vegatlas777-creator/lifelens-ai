    ...
    <div className="px-5 space-y-4">
      <div className="rounded-3xl bg-white border border-[#FFC0D6] shadow-sm shadow-pink-200/60 p-5">
        <div className="flex items-center gap-2 mb-3">
          <Ruler size={16} className="text-[#FF149C]" />
          <h3 className="text-sm font-bold text-[#4A0E2E] font-heading">Log Body Measurements (cm)</h3>
        </div>
        ...
        <button onClick={addEntry} disabled={adding} className="w-full mt-3 rounded-full bg-gradient-to-r from-[#FF69B4] to-[#FF149C] text-white py-3 font-semibold text-sm flex items-center justify-center gap-2 disabled:opacity-50 shadow-md shadow-pink-300/50">
      ...
      {entries.length > 0 ? (
        <div className="rounded-3xl bg-white border border-[#FFC0D6] shadow-sm shadow-pink-200/60 p-5">
          <h3 className="text-sm font-bold text-[#4A0E2E] mb-3 font-heading">Measurement History</h3>
          <div className="space-y-3">
            {entries.map((e, i) => (
              <div key={e.id || i} className="rounded-xl bg-[#FFF0F5] p-3">
                <p className="text-xs font-medium text-[#B0407A] mb-2">{formatDate(e.entry_date)}</p>
                <div className="grid grid-cols-5 gap-1 text-center">
                  {['chest_cm', 'waist_cm', 'hips_cm', 'arm_cm', 'thigh_cm'].map((k) => (
                    <div key={k}>
                      <p className="text-[9px] text-[#D67A9E] uppercase">{k.replace('_cm', '')}</p>
                      <p className="text-sm font-medium text-[#4A0E2E]">{e[k] ? `${e[k]}` : '—'}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="rounded-2xl border border-dashed border-[#FFC0D6] p-8 text-center">
          <p className="text-sm text-[#B0407A]">No measurements logged yet.</p>
        </div>
      )}
    </div>
  );
...
function Field({ label, children }) {
  return (<div><label className="text-[10px] font-medium text-[#B0407A] mb-1 block">{label}</label>{children}</div>);
}