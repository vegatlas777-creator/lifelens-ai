    ...
    <div className="min-h-screen bg-[#FDFBF8] pb-4">
      ...
      <div className="px-5 mt-2">
        ...
        <div className="relative overflow-hidden rounded-3xl">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-[#FF9F43]/85 to-[#E8821E]/70" />
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          ...
          <StatBox icon={Activity} value={todayStats.burned} label="kcal burned" color="text-[#FF9F43]" />
        ...
        <div className="mt-4">
          {subStatus.isPremium ? (
            <div className="rounded-2xl bg-white border border-[#FF9F43]/30 p-4 flex items-center gap-3">
              <div className="p-2.5 rounded-xl bg-[#FDDDBD]">
                <Crown size={20} className="text-[#E8821E]" />
              </div>
              <div className="flex-1">
                <p className="font-semibold text-sm text-[#1A1A1A]">Premium Active</p>
                <p className="text-xs text-[#666]">{subStatus.subscription?.billing_cycle === 'annual' ? 'Annual plan' : 'Monthly plan'}</p>
              </div>
            </div>
          ) : (
            <Link to="/pricing" className="relative overflow-hidden rounded-2xl block">
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-[#1A1A1A]/85 to-[#1A1A1A]/50" />
        ...
        {profile && (
          <div className="mt-4 rounded-2xl bg-white border border-[#F5EFE6] p-4">
            <p className="text-xs font-medium text-[#666] mb-3">METABOLIC PROFILE</p>
        ...
        {/* Settings */}
        <div className="mt-4 rounded-2xl bg-white border border-[#F5EFE6] divide-y divide-[#F5EFE6]">
          <button onClick={toggleDark} className="w-full flex items-center gap-3 p-4 hover:bg-[#FDF6EE] transition-colors">
            <div className="p-2 rounded-xl bg-[#FDDDBD]">
              {dark ? <Moon size={18} className="text-[#E8821E]" /> : <Sun size={18} className="text-[#E8821E]" />}
            </div>
            <span className="flex-1 text-left text-sm font-medium text-[#1A1A1A]">Dark Mode</span>
            <div className={`w-10 h-6 rounded-full transition-colors ${dark ? 'bg-[#FF9F43]' : 'bg-[#F5EFE6]'}`}>
        ...
        <div className="mt-6 text-center">
          <p className="text-xs text-[#666]">3 in 1 Healthy Choice · v2.0</p>
          <p className="text-[10px] text-[#999] mt-1">All estimates are approximations and not medical advice.</p>
        </div>
      </div>
    </div>
  );
...
function Info({ label, value }) {
  return (
    <div>
      ...
      <p className="font-semibold text-sm text-[#1A1A1A]">{value}</p>
    </div>
  );
}