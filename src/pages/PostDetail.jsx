...
export default function PostDetail() {
  ...
  if (!post) {
    return (
      <div className="min-h-screen bg-[#FDFBF8] flex flex-col items-center justify-center px-5">
        <p className="text-sm text-[#666]">Post not found.</p>
        <Link to="/community" className="mt-3 px-4 py-2 rounded-full bg-[#FFD5A8] text-[#1A1A1A] text-sm font-semibold">Back to Community</Link>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF8] pb-4">
      {/* Header */}
      <div className="px-5 pt-12 pb-3 flex items-center gap-3 border-b border-[#F5EFE6]">
        <button onClick={() => navigate('/community')} className="p-1.5 rounded-full hover:bg-[#FDF6EE]"><ArrowLeft size={20} className="text-[#1A1A1A]" /></button>
        <h1 className="text-lg font-bold text-[#1A1A1A]">Discussion</h1>
      </div>
        ...
        <div className="rounded-3xl bg-white border border-[#F5EFE6] overflow-hidden">
          <div className="p-4">
            <div className="flex items-center gap-2 mb-3">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#FFD5A8] to-[#FF9F43] flex items-center justify-center text-white text-sm font-bold">
                {post.author_name?.[0]?.toUpperCase() || 'U'}
              </div>
              <div className="flex-1">
                <p className="text-sm font-semibold text-[#1A1A1A]">{post.author_name || 'Member'}</p>
                <p className="text-[10px] text-[#999]">{timeAgo(post.created_date)}</p>
              </div>
              <span className="px-2.5 py-1 rounded-full bg-[#FDF6EE] text-[10px] font-medium text-[#666]">{cat.emoji} {cat.label}</span>
            </div>
            <h2 className="text-lg font-bold text-[#1A1A1A] mb-2">{post.title}</h2>
            <p className="text-sm text-[#666] leading-relaxed whitespace-pre-wrap">{post.content}</p>
          </div>
          {post.image_url && (
          ...
          <div className="flex items-center gap-1 px-3 py-2 border-t border-[#F5EFE6]">
            <button onClick={toggleLike} className={`flex items-center gap-1.5 px-3 py-2 rounded-full text-sm font-medium ${liked ? 'text-rose-500 bg-rose-50' : 'text-[#666] hover:bg-[#FDF6EE]'}`}>
              <Heart size={16} fill={liked ? 'currentColor' : 'none'} /> {likeCount > 0 && likeCount}
            </button>
            <button onClick={toggleFollow} className={`flex items-center gap-1.5 px-3 py-2 rounded-full text-sm font-medium ${following ? 'text-[#FF9F43] bg-[#FDDDBD]/30' : 'text-[#666] hover:bg-[#FDF6EE]'}`}>
              {following ? <BellOff size={16} /> : <Bell size={16} />} {following ? 'Following' : 'Follow'}
            </button>
            <button onClick={reportPost} className="ml-auto px-3 py-2 rounded-full text-sm text-[#999] hover:bg-[#FDF6EE]">
              <Flag size={16} />
            </button>
          </div>
        </div>
      ...
      {isPremium && (
        <div className="px-5 mt-4">
          <div className="rounded-2xl bg-[#FDDDBD]/30 border border-[#FDDDBD] p-3 flex items-center gap-2">
            <Crown size={14} className="text-[#E8821E]" />
            <p className="text-xs text-[#666]">You can add photos to your comments!</p>
          </div>
        </div>
      )}
