    ...
    <div className="min-h-screen bg-[#FDFBF8] pb-4">
      ...
      <div className="px-5 pt-12 pb-3">
        <div className="flex items-center gap-2">
          <div className="w-9 h-9 rounded-xl bg-[#FDDDBD] flex items-center justify-center">
            <Users size={20} className="text-[#E8821E]" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-[#1A1A1A]">Community</h1>
            <p className="text-[11px] text-[#666]">Connect, share & grow together</p>
          </div>
        </div>
      </div>
      ...
      {dailyTopic && (
        <div className="px-5 mt-2">
          <div className="rounded-2xl bg-gradient-to-r from-[#FFD5A8] to-[#FF9F43] p-4 text-white">
      ...
      <div className="px-5 mt-3">
        <div className="flex items-center gap-2 rounded-full bg-white border border-[#F5EFE6] px-4 py-2.5">
          <Search size={16} className="text-[#999]" />
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search discussions..." className="flex-1 bg-transparent text-sm focus:outline-none text-[#1A1A1A]" />
          {search && <button onClick={() => setSearch('')}><X size={14} className="text-[#999]" /></button>}
        </div>
      </div>
      ...
      <div className="mt-3 px-5">
        <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
          <button onClick={() => setActiveCat('all')} className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap ${activeCat === 'all' ? 'bg-[#FF9F43] text-white' : 'bg-white border border-[#F5EFE6] text-[#666]'}`}>All</button>
          {CATEGORIES.map((c) => (
            <button key={c.value} onClick={() => setActiveCat(c.value)} className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap ${activeCat === c.value ? 'bg-[#FF9F43] text-white' : 'bg-white border border-[#F5EFE6] text-[#666]'}`}>{c.emoji} {c.label}</button>
          ))}
        </div>
      </div>
      ...
      <div className="px-5 mt-3">
        {!showForm ? (
          <button onClick={() => setShowForm(true)} className="w-full rounded-full bg-[#FFD5A8] text-[#1A1A1A] py-3 font-semibold text-sm flex items-center justify-center gap-2">
      ...
      <div className="px-5 mt-3 flex items-center justify-between">
        <button onClick={() => setShowGuidelines(true)} className="flex items-center gap-1 text-xs text-[#666]">
          <Shield size={13} /> Community Guidelines
        </button>
        <span className="text-xs text-[#999]">{filtered.length} posts</span>
      </div>

      {/* Posts */}
      <div className="px-5 mt-3 space-y-3">
        {loading ? (
          <div className="flex justify-center py-8"><Loader2 size={28} className="text-[#FF9F43] animate-spin" /></div>
        ) : filtered.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-[#FDDDBD] p-8 text-center">
            <p className="text-sm text-[#666]">No discussions yet. {search ? 'Try a different search.' : 'Be the first to post!'}</p>
          </div>
        ) : (
          filtered.map((p) => (
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40" onClick={() => setShowGuidelines(false)}>
          <div className="w-full max-w-md bg-white rounded-t-3xl p-5 max-h-[70vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-base font-bold text-[#1A1A1A] flex items-center gap-2"><Shield size={18} className="text-[#FF9F43]" /> Community Guidelines</h2>
              <button onClick={() => setShowGuidelines(false)}><X size={18} className="text-[#666]" /></button>
            </div>
            ...
            <p className="text-[11px] text-[#999] mt-4">⚠️ This community is for educational support only. Content is not medical advice. Violations may result in content removal.</p>
          </div>
        </div>
      )}
    </div>
...
function Guideline({ text }) {
  return <div className="flex items-start gap-2"><ChevronRight size={14} className="text-[#FF9F43] mt-0.5 flex-shrink-0" /><p>{text}</p></div>;
}